...
export async function fetchUserThreads(userId: string) {
  try {
    ...
    throw new Error(`Error to fetch user threads: ${error.message}`);
  }
}
...
export async function fetchUsers({
  ...
  try {
    ...
    throw new Error(`Error to fetch all user: ${error.message}`);
  }
}
...
export async function getActivity(userId: string) {
  try {
    ...
    const childThreads = await userThreads.reduce((acc, userThreads) => {
